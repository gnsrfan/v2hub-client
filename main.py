...
class ProxyApp:
    ...
    def toggle_connection(self):
        if not self.connected:
            ...
            subprocess.run(["gsettings", "set", "org.gnome.system.proxy.socks", "port", "1080"])
            subprocess.run(["gsettings", "set", "org.gnome.system.proxy.ignore-hosts", "['localhost', '127.0.0.0/8', '::1']"])
            #subprocess.run(["gsettings", "set", "org.gnome.system.proxy.http", "host", "''"])
            #subprocess.run(["gsettings", "set", "org.gnome.system.proxy.http", "port", "''"])
            #subprocess.run(["gsettings", "set", "org.gnome.system.proxy.https", "host", "''"])
            #subprocess.run(["gsettings", "set", "org.gnome.system.proxy.https", "port", "''"])
            #subprocess.run(["gsettings", "set", "org.gnome.system.proxy.ftp", "host", "''"])
            #subprocess.run(["gsettings", "set", "org.gnome.system.proxy.ftp", "port", "''"])
            # Run v2ray in a new terminal window
            self.process = subprocess.Popen(["bash", "-c", "./v2ray run"], stdout=subprocess.PIPE, stderr=subprocess.PIPE) 
            


            tk.messagebox.showinfo(title="Connected!", message=f"Your IP is secured by V2hub\n5.75.205.133 Germany")#{get('https://api.ipify.org').text}
            self.ip_label.config(text = self.ipn())
            ...
            # Close v2ray process
            subprocess.run(["pkill", "-f", "v2ray run"])

            tk.messagebox.showinfo(title="Disconnect!", message=f"Your IP is not secured anymore\n{get('https://api.ipify.org').text} Iran")
    def open_terminal(self):
        if not self.terminal_window:
            # Create new terminal window
            self.terminal_window = tk.Toplevel(self.root)
            self.terminal_window.title("Terminal")
            self.terminal_window.geometry("{}x{}+{}+{}".format(self.window_width, self.window_height, x_cordinate, y_cordinate))

            # Set window background color
            self.terminal_window.configure(bg="#666666")

            # Add text widget for displaying output
            self.output_text = tk.Text(self.terminal_window, state=tk.DISABLED, bg="#333333", fg="#ffffff", font=("Courier", 12))
            self.output_text.pack(expand=True, fill=tk.BOTH, padx=10, pady=10)

            # Add entry widget for input
            self.input_entry = tk.Entry(self.terminal_window, bg="#333333", fg="#ffffff", font=("Courier", 12))
